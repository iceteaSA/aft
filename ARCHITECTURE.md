...
2. Evict root-scoped artifact handles (callgraph store, search index, semantic index, and inspect caches) from memory via `evict_idle_artifacts`, leaving them to be lazily re-opened on subsequent queries -- `crates/aft/src/context.rs`. Stop the file watcher runtime in the background.
...
4. Track process-wide and root-scoped memory usage (including SQLite allocator metrics and OS RSS memory) via memory snapshots returned in status reports -- `crates/aft/src/memory.rs`, `crates/aft/src/commands/status.rs`.
