...
1. Index project files using a disk-backed, pread-based trigram search index that keeps memory overhead bounded -- `crates/aft/src/search_index.rs`. To prevent redundant disk hashing and index re-verification loops during configure bind/warmup sequences, a verification memo with a 10-minute TTL manages cache freshness checks, utilizing metadata stat checks (`VerifyStrategy::StatFirst`) when possible rather than strict content hashing.
...
2. When a project root becomes unbound (no active routes/channels remaining and no pending binds), the subc daemon quiesces it: marks the actor context as subc unbound, invalidates the configure generation, retires search/callgraph/semantic build receivers, cancels queued and pending artifact work, cancels all queued maintenance jobs (returning `"maintenance_cancelled"` answers, except for active `Lsp` drains which are allowed to run/finish), stops the watcher runtime, and discards deferred configure maintenance. Receiver generation/epoch pairs prevent already-dequeued results from committing after teardown or replacement, while per-artifact publication epochs prevent superseded workers from publishing stale disk pointers. When a new route is bound, the root is reactivated, clearing the quiesced and evicted flags.
3. Evict root-scoped artifact handles (callgraph store, search index, semantic index, and inspect caches) from memory via `evict_idle_artifacts`. Subsequent queries (e.g. glob, grep, semantic search) trigger an asynchronous background reload of the evicted index files (using `trigger_search_index_reload_if_evicted` and `trigger_semantic_index_reload_if_evicted` serialized via `artifact_reload_lock` to prevent duplicate concurrent loaders) while serving the immediate request through the fallback walk. Stop the file watcher runtime in the background. Because edits during watcher downtime go unobserved, invalidate the verify memo (forcing `WarmVerifyPlan::Strict` re-verification on subsequent warm reloads) to catch same-size/preserved-mtime edits.
4. Under macOS, after sweeping idle roots, request memory pressure relief from the OS allocator via `relieve_allocator_pressure` to reclaim unused pages -- `crates/aft/src/memory.rs`.
5. Track process-wide and root-scoped memory usage (including SQLite allocator metrics and OS RSS memory) via memory snapshots returned in status reports -- `crates/aft/src/memory.rs`, `crates/aft/src/commands/status.rs`. Key status memory roots by `ProjectRootId` on all platforms to prevent path-casing/verbatim comparison mismatches.
